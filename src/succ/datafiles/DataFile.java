...
public class DataFile extends ReadableWritableDataFile implements IDataFileOnDisk, Runnable {
    ...
    @Override
    public synchronized void setSavedText(String text) {
        try {
            Files.write(Paths.get(filePath.get()), Collections.singleton(text), StandardOpenOption.TRUNCATE_EXISTING);
        } catch (IOException e) {
            throw new RuntimeException("Error while writing file " + filePath.get(), e);
        }
